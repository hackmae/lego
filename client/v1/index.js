...
// 2. Log the variable
var highestReduction = 'https://www.dealabs.com/bons-plans/75-sur-la-gamme-lego-mario-parthenay-79-2972397';
...
// 2. Log the average
var averageDiscount = deals.reduce((acc, deal) => acc + deal.discount, 0) / deals.length;
...
const communities = deals.reduce((acc, deal) => {
  if (!acc[deal.community]) acc[deal.community] = [];
...
// 2. Log the sort
console.log('\n\nDeals by price for each community');
for (const community in communities) {
  communities[community].sort((a, b) => b.price - a.price); // Highest to lowest
  console.log('Community:', community);
  communities[community].forEach(deal => {
...
// 2. Log the sort
console.log('\n\nDeals by date for each community');
for (const community in communities) {
  communities[community].sort((a, b) => new Date(a.published) - new Date(b.published)); // Old to recent
  console.log('Community:', community);
  communities[community].forEach(deal => {
    console.log('Title : ',deal.title, ' \nDate : ',deal.published);
  });
...
const prices = VINTED.map(item => parseFloat(item.price)); // Convert prices to numbers
...
console.log('P5 Price Value : ',p5Value.toFixed(2),'€');
console.log('P25 Price Value : ',p25Value.toFixed(2) ,'€');
...
console.log('\n\nLocalStorage : ',localStorage.getItem('MY_FAVORITE_DEALERS'));
